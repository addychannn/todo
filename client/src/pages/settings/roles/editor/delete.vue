  ...
  <TCard class="relative max-h-[95dvh] w-[95dvw] max-w-sm">
    <TCardHeader class="dark:!border-background/25">
      <TCardTitle class="flex items-center">
        <TIcon name="warning" class="mr-1 text-warning" /> Delete Role
      </TCardTitle>
      <TButton
        icon="close"
        iconSize="sm"
    ...
    <TCardFooter class="flex items-center gap-2 dark:!border-background/25">
      <label
...
const deleteRole = () => {
  if (answer.value === response) {
    loading.value = true;
    loadingMessage.value = "Deleting role, please wait...";
    $api
