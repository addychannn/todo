  ...
  <div class="flex h-full items-center">
    ...
    >
      <div class="flex items-center gap-1 text-lg font-semibold">
        <TIcon name="info" class="text-primary" /> Change Primary Address?
      </div>
      <div class="flex-auto">
        <p>
          You are about to change the primary address, would you like to
          continue?
        </p>
        <div>
          <div class="font-bold">Current Primary Address:</div>
          <div class="px-2">
            {{ primary.full }}
          </div>
          <div class="font-bold">New Primary Address:</div>
          <div class="px-2">
            {{ address.full }}
          </div>
        </div>
...
const changeMainAddress = () => {
  ...
  $api
    .put(`${props.api}/${props.address.id}`)
    .then((response) => {
